...
struct ComponentFactory : public TLocker
{
	...
	Array<uint16_t> Salts;
	Queue<uint16_t> FreeHandleQueue;
...
struct IGameObject : public ProtectedGameObject<IGameObject>
{
	...
	virtual void SetGUID_m(FixedString* fs) = 0;
	virtual FixedString* GetGUID_m() = 0;
	virtual void SetHandle(ObjectHandle handle) = 0; // TODO is this actually handle or netid?
	virtual ObjectHandle GetHandle() = 0;
	virtual void Unknown7() = 0;
	virtual GameObjectTemplate* GetCurrentTemplate() = 0;
	...
	virtual bool HasFlag(uint32_t flags) = 0;
...
namespace ecl
{
	...
	typedef void (*GameStateEventManager__ExecuteGameStateChangedEvent)(void* self, esv::GameState fromState, esv::GameState toState);
