...
struct TypedHandle
{
	static constexpr uint32_t NullHandle = 0ull;
	...
	explicit inline TypedHandle(uint32_t handle)
		: Handle(handle)
	{}

	explicit inline TypedHandle(int32_t handle)
		: Handle((uint64_t)handle)
	{}

	inline TypedHandle(uint32_t type, uint32_t index, uint32_t salt)
	{
		assert(type < 0x400 && salt < 0x400000);
		Handle = index | (salt << 32) | (type << 54); // TODO
	}
